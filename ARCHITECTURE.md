...
### memory — `src/memory/*` (`memorySql`, `rank`, `memoryStore`, `types`, `extractor`, `conversationHook`, `transcriptChunk`, `meetIngest`, `gmailIngest`, `ingestWatcher`), `src/llm/anthropicClient.ts`, `src/mcp/memory.ts`
