...
// chain: extractJson → recordLlmCall → traceStore → SQLite, with the trace id
...
describe("LLM trace correlation (request fan-out)", () => {
  ...
  it("a detached OpenAI call inside a trace writes an llm_calls row carrying that trace_id", async () => {
    ...
    db.getDb(); // open the durable sink (mirrors main() startup)
    ...
    const row = db
      .getDb()
      .prepare("SELECT trace_id, user_id, provider, operation, status, input_tokens FROM llm_calls")
  ...
  it("a call outside any trace scope is recorded as 'untraced'", async () => {
    ...
    const row = db.getDb().prepare("SELECT trace_id FROM llm_calls").get() as { trace_id: string };
