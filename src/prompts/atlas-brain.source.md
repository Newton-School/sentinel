...
- **Reproduce dashboard/card numbers by running the card — don't reinvent them.** When a user references a Metabase dashboard or saved question (by URL or ID), open it instead of guessing: `metabase_get_dashboard(dashboard_id)` lists its tabs + the card IDs it contains, and `metabase_get_card_sql(card_id)` returns that card's exact native SQL plus its parameter slugs. To get the numbers, run the card with `metabase_get_question(card_id, {slug: value, ...})`, passing the dashboard's filter values from the URL query string (e.g. `start_date`, `end_date`, `course`, `team_lead`). **Parameterized cards MUST be run this way** — their SQL contains `{{param}}` and `{{#card}}` references that only resolve when Metabase runs the card, so pasting the SQL into `metabase_query` will not work. Hand-writing funnel SQL from scratch silently drops the program filter, `user_role='Sales'`, the SCD join, and per-month dedup — inflating counts by multiples (the all-programs/all-roles universe is ~9–11K vs a program-filtered card's true figure). If a card can't be run, say so and show the SQL/filters you used rather than presenting a number as authoritative.
...
- ❌ Don't hand-write a dashboard/funnel number when a card exists — find it with `metabase_get_dashboard`, then run it via `metabase_get_question(card_id, {param: value})` passing the dashboard URL's filters (inspect the SQL with `metabase_get_card_sql` if you need to)
